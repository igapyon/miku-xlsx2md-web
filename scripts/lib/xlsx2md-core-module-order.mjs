export const XLSX2MD_CORE_JS_ORDER = [
  "src/js/module-registry.js",
  "src/js/module-registry-access.js",
  "src/js/runtime-env.js",
  "src/js/office-drawing.js",
  "src/js/zip-io.js",
  "src/js/border-grid.js",
  "src/js/markdown-normalize.js",
  "src/js/markdown-escape.js",
  "src/js/markdown-table-escape.js",
  "src/js/text-encoding.js",
  "src/js/markdown-options.js",
  "src/js/rich-text-parser.js",
  "src/js/rich-text-plain-formatter.js",
  "src/js/rich-text-github-formatter.js",
  "src/js/rich-text-renderer.js",
  "src/js/narrative-structure.js",
  "src/js/table-detector.js",
  "src/js/markdown-export.js",
  "src/js/sheet-markdown.js",
  "src/js/styles-parser.js",
  "src/js/shared-strings.js",
  "src/js/address-utils.js",
  "src/js/rels-parser.js",
  "src/js/worksheet-tables.js",
  "src/js/cell-format.js",
  "src/js/xml-utils.js",
  "src/js/sheet-assets.js",
  "src/js/worksheet-parser.js",
  "src/js/workbook-loader.js",
  "src/js/formula-reference-utils.js",
  "src/js/formula-engine.js",
  "src/js/formula-legacy.js",
  "src/js/formula-ast.js",
  "src/js/formula-resolver.js",
  "src/js/formula/tokenizer.js",
  "src/js/formula/parser.js",
  "src/js/formula/evaluator.js",
  "src/js/core.js"
];
